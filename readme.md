...
```
$ python3 sip4dzip_check.py [入力パス] [出力パス]
```
* 入力パス
    * チェック対象のZIPファイルまたはフォルダ
    * フォルダを指定した場合、フォルダ内の全てのZIPファイルをチェック対象とする。また、サブフォルダも再帰的にチェックします
* 出力パス
    * チェック結果の格納先フォルダ
    * チェックを実行した時刻の名称のサブフォルダを作成し、その中にSIP4D-ZIPファイルを解凍します
    * 上記フォルダ内に、チェック結果のファイルを出力します（ファイル名：00_report.txt）
...
* オブジェクト配列の「properties_value」を持つ
    ...
    * 属性定義ファイルのtypeと、properties_valueのtypeがある場合、属性定義のtypeを優先する
...
1. temp_column.jsonをコピーし、新しいファイル名（情報種別コード）を変更する
